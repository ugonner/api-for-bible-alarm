<?php
include_once $_SERVER['DOCUMENT_ROOT']. '/bona/user/usermessage/usermessage.class.php';
...
if(isset($_GET["getconversation"])){
    ...
    if(!isset($_SESSION["userid"])){
        ...
        header("Location:/bona/user/forms/loginform.html.php?output=".$error);
        exit();
    }
    $uid = $_SESSION["userid"];
    session_write_close();
    ...
    if($conversation = $usermsg->getConversation($uid,$senderid,$amtperpage,$pgn)){
        include_once $_SERVER["DOCUMENT_ROOT"]."/bona/user/usermessage/conversation.html.php";
        exit();
    }else{
...
if(isset($_GET["getunreadmessages"])){
    ...
    if(!isset($_SESSION["userdata"])){
        $error = 'Please Login First With Correct Email And Password Pair';
        header("Location:/bona/user/index.php?error=".$error);
        exit();
    }
    $uid = $_SESSION["userid"];
    session_write_close();
    ...
    if($messages = $usermsg->getUnreadMessages($uid,$amtperpage,$pgn)){
        ...
        include_once $_SERVER["DOCUMENT_ROOT"]."/bona/user/usermessage/inbox.html.php";
        exit();
    }else{
...
if(isset($_GET["getconversationbyajax"])){
    ...
    if(!isset($_SESSION["userdata"])){
        $error = 'Please Login First With Correct Email And Password Pair';
        header("Location:/bona/user/index.php?uid=".$senderid."&error=".$error);
        exit();
    }
    $uid = $_SESSION["userid"];
    session_write_close();
    ...
    for($i=0; $i<count($conversation); $i++){
        $t = "<div class='thumbnail'><p><a href='/bona/user/index.php?uid=".$conversation[$i][5]."'>
        ...
        if(isset($_SESSION["userid"])&&($_SESSION["userid"]==$conversation[$i][5])){
            ...
            <a href='/bona/user/usermesage/index.php?deletemessage&mid=".$conversation[$i][2]."'>
            Delete</a></button>
            <button id='editmsgbtn' class='btn-warning' data-toggle='collapse' data-target='#editmsgdiv'>
            Edit</button>
            <div class='collapse' id='editmsgdiv'>
               <div class='form-group'>
                   <form class='form-group' action='/bona/user/usermessage/index.php' method='POST'>
...
if(isset($_GET["deletemessage"])){
    ...
    if($usermsg->deleteMessage($mid,$senderid)){
        header("Location:/bona/user/usermessage/index.php?getconversation&senderid=".$rxid."&error=".$error);
        exit();
    }else{
        $error = "Message No Longer Exist OR You Cannot Be Identified Eith ThiS Message, So You Cannot Delete It";
        include_once $_SERVER["DOCUMENT_ROOT"]."/bona/includes/errors/error.html.php";
...
if(isset($_POST["sendmessage"])){
    ...
    if(!isset($_SESSION["userid"])){
        $error = 'Please Login First With Correct Email And Password Pair';
        header("Location:/bona/user/index.php?uid=".$rxid."&error=".$error);
        exit();
    }
    ...
    $usermsg = new Usermessage();
    if($usermsg->sendMessageToUser($detail,$dop,$senderid,$rxid,$marked)){
        header("Location:/bona/user/usermessage/index.php?getconversation&senderid=".$rxid."&pgn=".$pgn."&output=".$error);
        exit();
    }else{
        $error = "Message No Longer Exist OR You Cannot Be Identified Eith ThiS Message, So You Cannot Delete It";
        include_once $_SERVER["DOCUMENT_ROOT"]."/bona/includes/errors/error.html.php";
